...
//bool  Model::SetVertices(const VertexTypeA* in_vertices,  UINT in_count, UINT in_offset)
//{
...
//	ID3D11Device* device = m_graphics->GetDirectXDevice();
//
//	if (device == nullptr)
//	{
...
//	}
//
//	HRESULT HR(device->CreateBuffer(&vertexBufferDesc, &vertexData, g->m_verticesArray[in_offset].GetAddressOf()));
//
//	return true;
//}
//bool Model::SetIndices(ID3D11Device* device, const vector<DWORD> index, UINT offset, Model* g)
//{
...
//
//	HRESULT HR(device->CreateBuffer(&indexBufferDesc, &indexData, g->m_indexBuffers[offset].GetAddressOf()));
//
//	return true;
//}
//
...
//
//
//bool Model::Initialize(const std::string& filePath, ID3D11Device* device, ID3D11DeviceContext* deviceContext, ID3D11ShaderResourceView* texture, ConstantBuffer<ContantBuffer_VS>* cb_vs_vertexshader)
//{
//	this->m_device = device;
//	this->m_deviceContext = deviceContext;
//	this->m_VSConstantBuffer = cb_vs_vertexshader;
//
//
//	return true;
//}
...
//
////
//void Model::DrawMVP(const XMMATRIX& viewProjectionMatrix, ID3D11DeviceContext* deviceContext)
//{
//	this->m_VSConstantBuffer->data.worldMat = this->m_worldMatrix;
//	this->m_VSConstantBuffer->data.MVPMat = viewProjectionMatrix;
//	this->m_VSConstantBuffer->data.worldMat = this->m_worldMatrix;
//	this->m_VSConstantBuffer->data.MVPMat = viewProjectionMatrix;
//
//	this->m_VSConstantBuffer->data.MVPMat = XMMatrixTranspose(this->m_VSConstantBuffer->data.MVPMat);
//	
//	this->m_VSConstantBuffer -> ApplyChanges();
//
//	deviceContext->VSSetConstantBuffers(0, 1, this->m_VSConstantBuffer->GetAddressOf());
//	
//}
/////***********
...
////		}
////		
////		Model mesh = ProcessMesh(pMesh, pScene);
...
////
////
////	return Model(this->device, this->deviceContext, vertices, indices);
////}
//
//
//
...
////**************************************************
//
//Model::Model(ID3D11Device* device, ID3D11DeviceContext* _deviceContext, std::vector<Vertex>& vertices, std::vector<DWORD>& indices)
//{
//	this->m_deviceContext = _deviceContext;
//
//	HRESULT hr = this->m_vertexbuffer.Initialize(device, vertices.data(), vertices.size());
//	if (FAILED(hr))
//	{
...
//
//
//	hr = this->m_indexBuffers.Initialize(device, indices.data(), indices.size());
//	if (FAILED(hr))
//	{
...
//{
//	this->m_deviceContext = model.m_deviceContext;
//	this->m_indexBuffers = model.m_indexBuffers;
//	this->m_vertexbuffer = model.m_vertexbuffer;
//}
//
//
//
...
//{
//	UINT offset = 0;
//	this->m_deviceContext->IASetVertexBuffers(0, 1, this->m_vertexBuffers[0].GetAddressOf(), &m_meshTable[0]->m_vertexStride, &offset);
//	this->m_deviceContext->IASetIndexBuffer(m_indexBuffers[0].Get(),DXGI_FORMAT_R32_UINT,offset);
//	this->m_deviceContext->DrawIndexed(m_meshTable[0]->m_faceCount * 3, 0, 0);
//}
//
...
//	{
//		m_meshTable[i]->Draw(deviceContext, inputLayout, vShader, pShader,i,this);
//	}
//
//
//}
//
//bool Model::Initalize(ID3D11Device* in_device, ID3D11DeviceContext* in_deviceContext, std::vector<Vertex>& in_vertices, std::vector<DWORD>& in_indices)
//{
//	m_device = 
//
//	return false;
...
//
//
//bool Model::Initalize(Graphics* in_graphics, std::vector<Vertex>& in_vertices, std::vector<DWORD>& in_indices)
//{
//	m_graphics = in_graphics;
//
//
//
//}
...
//			}
//			mesh.m_pVertices.push_back(temp);
//
//
//		}
//
...
//
//
//bool Model::LoadMeshesWithSkinnedAnimation(const std::string& filePath)
//{
...
//	for (int i = 0; i < assimpScene->mNumMeshes; i++)
//	{
//		m_meshTable.emplace_back();
//		Model& model = m_meshTable.back();
